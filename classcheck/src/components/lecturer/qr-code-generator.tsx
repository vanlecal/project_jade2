...
export function QrCodeGenerator() {
  ...
  const generateAndSetQr = async () => {
    try {
      ...
      // Generate QR with location data
      const { code, expiresAt } = await generateQr(title, program, lat, lon);
      ...
      if (err instanceof GeolocationPositionError) {
        setError("Please enable location access to generate the QR code.");
